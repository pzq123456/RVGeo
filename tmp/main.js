...
let cs10Btn = document.querySelector('.cs10');
cs10Btn.addEventListener('click', () => {
    // clear the canvas before drawing
    myCanvas.ctx.clearRect(0, 0, myCanvas.width, myCanvas.height);
    ...
    $.get(url2,function(data1,status){
      ...
      let grid3 = RV.Raster.fromMatrix(grid2.getAccumulationFlow());

      let gridview2 = new RV.Renderer.GridView(myCanvas.ctx,grid3,50,1050,950,150);
      // let gridview3 = new RV.Renderer.GridView(myCanvas.ctx,grid3,50,1050,950,150);
      ...
      // gridview3.draw_dispersed_custom(myCanvas.height,true,RV.pan.CellValueRenderer.Gray,"坡向测试视图",9,1,0.1,[0,1,2,4,8,16,32,64,128]);


      grid3.reClassify_Binary_(5)
      let stt2 =new RV.Stastic(grid3.get1DArray());
