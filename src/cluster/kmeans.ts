...
import { randomIndexArray } from "../core";
...
export function K_means(
    ...
    */
    ...
    // 1.从样本中选择 K 个点作为初始质心（完全随机）
    let centroids = [] as [number,number][];
    let len = points.length;
    if(len < k){
    ...
    indexArray.forEach((item) => {
        let tmp = points[item] as [number,number];
        centroids.push(tmp);
        }
    );
    ...
    while(dc > thresh && times < maxtime){
        ...
        for(let i = 0 ; i < len ; i++){
            ...
            for(let j = 0 ; j < k ; j++){
                let tmp1 = points[i] as [number,number];
                let tmp2 = centroids[j] as [number,number];
        ...
        // 3.计算每个簇内所有样本的均值，并使用该均值更新簇的质心
        let new_centroids = [] as [number,number][];
        for(let i = 0 ; i < k ; i++){
        ...
        dc = 0;
        for(let i = 0 ; i < k ; i++){
            let tmp1 = centroids[i] as [number,number];
            let tmp2 = new_centroids[i] as [number,number];
            let dis = haversine(tmp1,tmp2);
            if(dis > dc){
                dc = dis;
            }
